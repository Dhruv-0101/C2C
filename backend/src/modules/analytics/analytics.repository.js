...
export const analyticsRepository = {
   ...
   */
  getDailyTrends: async (userId, startDate, platform = null) => {
    const where = {
      userId,
      createdAt: { gte: startDate },
    };
   ...
   */
  getTopTemplates: async (userId, limit = 5) => {
    const items = await prisma.postAnalytics.findMany({
      where: { userId },
      take: 20,
   ...
   */
  seedDemoAnalytics: async (userId) => {
    let posts = await prisma.post.findMany({
      where: { userId },
      take: 10,
    });

    // If user has no posts yet, create mock posts so analytics can be visualized
    if (posts.length === 0) {
      const mockPostsData = [
    ...
    for (let i = 0; i < posts.length; i++) {
      ...
      for (const platform of platforms) {
        // Generate random realistic dates over the past 30 days
