...
import { sanitizeFrame, sanitizeFrames } from './frame.helper.js';
import {
...
export async function createFrame(payload = {}, fileBuffer = null, createdBy = null) {
  const targetBuffer = fileBuffer || payload.fileBuffer || null;
  ...
  if (targetBuffer) {
    const uploadResult = await uploadFrameBuffer(targetBuffer);
