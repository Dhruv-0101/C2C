...
export const liveSocialPublisherService = {
   ...
   */
  publishToPlatforms: async ({ postId, userId, postContent, graphicUrl, targetPlatforms }) => {
    logger.info(`🚀 [LiveSocialPublisher] Routing publish job for Post ID: ${postId} across: ${targetPlatforms.join(', ')}`);
    ...
    for (const platform of targetPlatforms) {
      ...
      if (platformUpper === 'INSTAGRAM') {
        try {
          ...
          if (isLiveMode && igAccount && igAccount.isConnected && igAccount.accessToken) {
            logger.info(`🌐 [LiveSocialPublisher] Publishing to Live Instagram Account (@${igAccount.accountName})...`);
            ...
            logger.info(`ℹ️ [LiveSocialPublisher] No active live token found for Instagram (Mode: ${env.SOCIAL_PUBLISHER_MODE}). Executing mock publish.`);
          ...
          logger.error('❌ [LiveSocialPublisher] Live Instagram publishing failed:', err.message);
    ...
    const hasSuccess = Object.values(platformResults).some((r) => r.status === 'SUCCESS');
